...
export async function fetchData() {
  const url =
    "https://ecitulje.com/api/v1/ceremony?type=FUNERAL&ceremonyDate=2026-01-29T23:00:00.000Z";
...
(async () => {
  try {
    ...
    const dataOrl = data.results[4];
    if (!dataOrl) {
      console.error(
        "data.results[4] is undefined. data.results:",
        data.results,
      );
      return;
    }
    ...
    if (!dataOrl.cemetery) {
      console.error("data.results[4].cemetery is undefined. dataOrl:", dataOrl);
      return;
    }
